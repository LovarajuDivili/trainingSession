...
const EditEmployeeDialog: React.FC<EditEmployeeDialogProps> = ({
  ...
  const handleSkillKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter" || e.key === ",") {
      if (
        skillInput.trim() &&
        !editEmployee.skills.includes(skillInput.trim())
      ) {
        e.preventDefault();
          ...
          <Box sx={{ display: "flex", gap: 2 }}>
            <Box sx={{ flex: 1 }}>
              ...
              <TextField
                ...
                onChange={(e) =>
                  setEditEmployee({ ...editEmployee, joinDate: e.target.value })
                }
      ...
      <DialogActions sx={{ justifyContent: "flex-end", px: 3, pb: 3 }}>
        ...
        <Button
          ...
          sx={{
            ...
            "&:hover": {
              backgroundColor:
                isFormValid && hasChanged ? primaryColor : "#d3d3d3",
            },
          }}
        >
          Update Employee
