...
const ProjectsLayout: React.FC = () => {
  ...
  const handleEmployeeSort = (field: string, direction: "asc" | "desc") => {
    if (employeeSortModel.length === 1 && employeeSortModel[0].field === field) {
      const newSort =
        employeeSortModel[0].sort === direction ? [] : [{ field, sort: direction }];
  ...
  return (
    <Box sx={{ flexGrow: 1, minHeight: "100vh", bgcolor: "#f8f9fa" }}>
      ...
      <Dialog open={isAdding} onClose={handleCloseClick} maxWidth="md" fullWidth>
        <DialogTitle sx={{ pr: 6 }}>
          Add New Project
          <IconButton
