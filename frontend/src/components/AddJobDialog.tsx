...
const AddJobDialog: React.FC<AddJobDialogProps> = ({
  ...
  return (
    <Dialog
      open={open}
      onClose={onClose}
      PaperProps={{ sx: { width: '55vw', maxWidth: 900, borderRadius: '12px', p: '8px 0' } }}
    >
      <DialogTitle sx={{ display: 'flex', justifyContent: 'space-between', fontSize: '1.4rem', px: 3, pt: 2 }}>
        Add Job Opening
      ...
      <DialogContent sx={{ px: 3 }}>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          ...
          <Box sx={{ display: 'flex', gap: 2 }}>
            <Box sx={{ flex: 1 }}>
              <Typography sx={{ fontSize: 13, mb: 0.5 }}>Job Title *</Typography>
              <TextField
                fullWidth
                variant="outlined"
                placeholder="Enter job title"
                value={newJob.title}
                onChange={e => setNewJob((prev: any) => ({ ...prev, title: e.target.value }))}
                sx={textFieldSx}
              />
            </Box>
            ...
            <Box sx={{ flex: 1 }}>
              ...
              <TextField
                ...
                onChange={e => setNewJob((prev: any) => ({ ...prev, location: e.target.value }))}
                sx={textFieldSx}
              />
            </Box>
          </Box>
          ...
          <Box>
            ...
            <TextField
              ...
              onChange={e => setNewJob((prev: any) => ({ ...prev, description: e.target.value }))}
              sx={textFieldSx}
            />
          </Box>
          ...
          <Box sx={{ display: 'flex', gap: 2 }}>
            <Box sx={{ flex: 1 }}>
              <Typography sx={{ fontSize: 13, mb: 0.5 }}>Number of Openings *</Typography>
              <TextField
                fullWidth
                variant="outlined"
                type="number"
                ...
                onChange={e =>
                  setNewJob((prev: any) => ({ ...prev, openings: Number(e.target.value) }))
                }
                sx={textFieldSx}
              />
            </Box>
