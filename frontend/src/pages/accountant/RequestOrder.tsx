...
const RequestOrder: React.FC = () => {
  ...
  const categories = ["Laptop", "Monitor", "Keyboard", "Mouse", "Headphones", "Webcam", "Others"];
  ...
  return (
    <Box>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        {categories.map(cat => (
          <Grid item key={cat}>
            <Paper sx={categoryBox(activeCategory === cat)}
      ...
      <Grid container spacing={3}>
        {products.map((item: any) => (
          <Grid item xs={12} sm={6} md={4} lg={3} key={item._id}>
            <Card sx={{ borderRadius: 3, boxShadow: "0 8px 18px rgba(0,0,0,0.08)" }}>
              ...
              <CardActions sx={{ display: "flex", justifyContent: "space-between", px: 2 }}>
                <Box sx={{ cursor: "pointer" }} onClick={() => toggleFav(item._id)}>
